  ...
  static double pop(std::vector<double>& v) {
    ...
    void SCF::do_plots(World& world) {
        ...
        if (param.plotdens || param.plotcoul) {
            ...
            plotdx(rho, "total_density.dx", param.plot_cell, npt, false);
            if (param.plotcoul) {
    ...
    void SCF::solve(World & world) {
		...
		if (param.localize_pm)
		  ...
		  if (param.localize_pm)
            ...
            if (iter > 0) {
		print("amo.size() = ", amo.size());
		print("dconv * natom =", dconv * molecule.natom());
		print("5.0 * dconv = ", 5.0*dconv);
...
//Xing OEP
    double SCF::solve(World & world, const functionT& vnuc, const functionT& vcoul,functionT& vxc, const functionT& refrho) {
        PROFILE_MEMBER_FUNC(SCF);
	...
	double Ws;
	    ...
	    Vpsia = mul_sparse(world, vlocal, amo, vtol);
	    truncate(world, Vpsia);
	    world.gop.fence();
            if (!param.spin_restricted && param.nbeta) {
                Vpsib = mul_sparse(world, vlocal, bmo, vtol);;
...
void plot_radial_function(const real_function_3d& rho, const std::string filename) {
    ...
    double SCF::line_search(double& W, const double Win, const functionT& xin, const functionT& s, const functionT& g,
    ...
    void SCF::update_potential(World& world, int iter, double& W, const double Win,
	...
	if(iter == 1){
	   s = -1.0 * g;
	   alpha = line_search(W, Win, x, s, g, world, vnuc, vcoul, refrho);
	   x += alpha * s;
	   return;
        }
    ...
    functionT SCF::oep(World & world, const functionT& rho, const functionT& brho)
    {
        PROFILE_MEMBER_FUNC(SCF);
        START_TIMER(world);

        functionT trial_pot = real_factory_3d(world).empty();

	int proto_size = param.protocol_data.size();
	set_protocol<3>(world,param.protocol_data[proto_size-1]);

	int ispin = 0;
	//initial guess of potential
//        XCOperator xcoperator(world,"LDA",ispin,rho,brho);
//        trial_pot=xcoperator.make_xc_potential();

	functionT vcoul  = apply(*coulop, rho);
        functionT vnuc   = potentialmanager->vnuclear();

	int n_elec = param.nalpha + param.nbeta;
	double factor = -1.0/(double)n_elec;
	trial_pot = factor * vcoul;
	...
	vecfuncT orbitals=copy(world,amo);
        tensorT eps=aeps;
        int iter = 0;
        int max_iter = 100;
        bool cnvgd = false;
	const double dconv = std::max(FunctionDefaults < 3 > ::get_thresh(),
                                      param.dconv);
        double oep_thresh = std::max(dconv, param.oepconv);
	functionT x, x_old, g_old, g, s_old, s;
        x=trial_pot;
	double w, w_old;
	   ...
	   if(iter == 1)
	     w = solve(world, vnuc, vcoul, x, rho);

           // make the density from the orbitals
           functionT trial_dens=2.0*make_density(world,aocc,amo);

           // print out the status
           functionT g = rho - trial_dens;
	   ...
	   if(cnvgd) break;
	   w_old = w;
           update_potential(world,iter,w,w_old,g_old,g,x_old,x,s_old,s, vnuc, vcoul, x, rho);
	...
	//functionT vxc = trial_pot;
	//vxc -= (1.0/factor*(1.0-factor)) * vcoul;
